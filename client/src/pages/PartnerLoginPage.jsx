...
export default function PartnerLoginPage() {
  ...
  const [form, setForm] = useState({ email: 'partner@demo.com', password: 'demo@123' });
  const [showPw, setShowPw] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const handleFillDemo = () => {
    setForm({ email: 'partner@demo.com', password: 'demo@123' });
    setError('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.email || !form.password) {
      setError('Please enter partner email and password.');
      return;
    }
    ...
    try {
      ...
      if (user && user.role !== 'partner' && user.role !== 'admin') {
        toast.error('Account is not authorized for Partner Portal');
        navigate('/dashboard');
        return;
      }
      ...
      setError(err.response?.data?.message || 'Invalid partner credentials');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between items-center py-6 px-4 font-sans relative overflow-hidden">
      ...
      <div className="text-center space-y-1 z-10">
        ...
        <p className="text-xs text-purple-400 font-bold uppercase tracking-wider">Scholarship Provider Operations</p>
      </div>

      {/* Login Card */}
      <div className="w-full max-w-md bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl z-10 space-y-6">
        <div>
          <h2 className="text-xl font-extrabold text-white">🏢 Partner Sign In</h2>
          <p className="text-xs text-slate-400 mt-1">Manage your scholarships, student applications, and document verification.</p>
        </div>

        {/* Demo Credentials Box */}
        <div className="bg-purple-950/40 border border-purple-800/40 rounded-xl p-3 flex items-center justify-between gap-2 text-xs">
          <div>
            <span className="font-bold text-purple-400">🧪 Demo Partner:</span>
            <p className="font-mono text-slate-300 text-[11px]">partner@demo.com • demo@123</p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Partner Email</label>
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              ...
              <button
                type="button"
                onClick={() => toast.info('Contact UniScholar partner team to reset credentials.')}
                className="text-[11px] font-bold text-slate-400 hover:text-purple-400"
              >
                Forgot Password?
              </button>
            </div>
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••"
                className="w-full pl-10 pr-10 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-purple-500"
                required
              />
              <button
                type="button"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-extrabold text-xs rounded-xl shadow-lg shadow-purple-500/20 transition-all flex items-center justify-center gap-2"
          >
            {loading ? <Spinner /> : 'Sign In to Partner Portal →'}
          </button>
        </form>
